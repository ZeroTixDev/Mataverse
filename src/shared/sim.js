...
const Weapons = {
	...
	LMG: { cooldown: 0.13, gunWidth: 6.5, gunHeight: 2, ammo: 40, reloadTime: 4.5, err: 35, color: '#00332d', recoil: 0},
	Energy: { cooldown: 0.18, gunWidth: 6, gunHeight: 2, ammo: 25, reloadTime: 3, err: 10, color: '#7b2ff2', recoil: 0},
}
...
function simPlayer(player, inputPayload, delta, players, arena, obstacles=[]) {
	...
	if (!player.skating || (player.skating && player.shiftTime <= 0.2)) {
		// very light damping: speed builds up and carries - momentum matters
    	_xv *= 0.96;
	    _yv *= 0.96
	}
