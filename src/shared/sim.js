...
const Weapons = {
	...
	Burst: { cooldown: 1.1, gunWidth: 4, gunHeight: 2.4, ammo: 18, reloadTime: 2.75, err: 20, color: '#6e0400', recoil: 1, rrAmmo: 9},
...
function simPlayer(player, inputPayload, delta, players, arena, obstacles=[]) {
	...
	_shiftRegenTimer += dt;
		if (_shiftRegenTimer >= player.shiftRegenTime) {
			let mult = player.powers.includes('Ice Skate') ? 0.75: 1;
			_currentShift += dt*1.2*mult;
